...
KEYWORDS = {"if", "else", "void", "int", "for", "return", "break"}
...
class Parser:
    ...
    def _token_for_tree(self) -> Node:
        ...
        if ttype in ("ID", "NUM"):
            ...
            return Node(f"(KEYWORD, {tval})")
        else:
            return Node(f"(SYMBOL, {tval})")
    ...
    # ========== Helper methods for recursive descent ==========
    
    def _match(self, terminal: str, parent_node: Node) -> bool:
        """
        Match a terminal symbol. If it matches, advance and add token node.
        If it doesn't match, report missing terminal error.
        ...
        if la == terminal:
            # Match - create properly formatted token node and advance
            ttype, tval, _ = la_tok
            if ttype in ("ID", "NUM"):
                token_str = f"({ttype}, {tval})"
            elif ttype == "KEYWORD":
                token_str = f"(KEYWORD, {tval})"
            else:
                token_str = f"(SYMBOL, {tval})"
            
            tok_node = Node(token_str)
            tok_node.matched = True
            parent_node.add(tok_node)
            self._advance()
            return True
        else:
            # Terminal mismatch - report missing terminal
            self.errors.append(f"#{line} : syntax error, missing {terminal}")
            missing_node = Node(f"missing {terminal}")
            missing_node.hidden = True
            missing_node.matched = False
            parent_node.add(missing_node)
            return False

    def _parse_nonterminal(self, nonterminal: str, parent_node: Node) -> bool:
        """
        Parse a nonterminal using the parse table.
        Returns True if parsing was successful, False if error recovery occurred.
        """
        la_tok = self._lookahead_token()
        la = self._la_symbol()
        line = la_tok[2]
        ...
        if key in self.table:
            ...
            if not rhs:
                ...
                parent_node.add(epsilon_node)
                return True
            
            # Process each symbol in the production
            for sym in rhs:
                if sym.kind == "T":
                    # Terminal - match and create token node directly
                    self._match(sym.name, parent_node)
                else:
                    # Nonterminal - create node and recurse
                    child = Node(sym.name)
                    parent_node.add(child)
                    self._parse_nonterminal(sym.name, child)
            
            return True
        else:
            # No table entry - error recovery
            if la in self.follow[nonterminal] or la in self.sync_tokens.get(nonterminal, set()):
                # Missing nonterminal - pop it
                if nonterminal != "Compound-stmt":  # Don't hide Compound-stmt
                    self.errors.append(f"#{line} : syntax error, missing {nonterminal}")
                    parent_node.abort = True
                    parent_node.hidden = True
                return False
            else:
                # Illegal token - skip it and try again
                if la == END:
                    ...
                    if la_tok[0] in ("ID", "NUM"):
                        ...
                        self.errors.append(f"#{line} : syntax error, illegal {la_tok[1]}")
                    self._advance()
                    # Try parsing the same nonterminal again with new lookahead
                    return self._parse_nonterminal(nonterminal, parent_node)

    # ========== Main parse entry point ==========
    
    def parse(self) -> Node:
        """
        Main parse method using recursive descent.
        Returns the parse tree root.
        """
        root = Node(self.start_symbol)
        self._parse_nonterminal(self.start_symbol, root)
